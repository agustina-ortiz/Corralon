    ...
    <style>
        * { font-family: DejaVu Sans, sans-serif; }
        ...
        /* table-layout fixed + anchos explícitos: mantiene las columnas alineadas
           entre las distintas tablas (una por chunk) y evita el desbordamiento de
           memoria de dompdf con una sola tabla gigante. */
        ...
        td { padding: 3px 5px; border-bottom: 1px solid #f0f0f0; word-wrap: break-word; }
...
<body>
            ...
            <tbody>
                @foreach($chunk as $m)
                    ...
                    <tr>
                        ...
                        <td>{{ $m->insumo->insumo ?? '' }}</td>
                        <td class="num">{{ number_format($m->cantidad, 2) }}</td>
                        <td>{{ $m->insumo->unidad ?? '' }}</td>
                        <td>{{ $m->insumo->deposito->deposito ?? '' }}</td>
                        <td>{{ $destinos[$m->id] ?? '' }}</td>
    ...
    @empty
        <table>
            <tbody>
                <tr><td style="text-align:center; padding:12px; color:#9ca3af;">No hay movimientos para los filtros seleccionados.</td></tr>
            </tbody>
        </table>
    @endforelse
