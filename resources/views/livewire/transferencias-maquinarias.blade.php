...
<div>
    ...
    {{-- =====================================================
         MODAL NUEVA TRANSFERENCIA (multi-maquinaria)
         ===================================================== --}}
    @if($showModalTransferencia)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-transferencia-title" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">Transfiera maquinarias entre depósitos</p>
                                    </div>
                                </div>
                                <button type="button" wire:click="cerrarModalTransferencia" class="text-gray-400 hover:text-gray-600 transition-colors">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
                                </button>
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select wire:model.live="id_corralon_origen" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="id_corralon_destino" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            wire:model.live="id_deposito_origen_tf"
                                        ...
                                        >
                                            ...
                                            @foreach($depositosOrigenTf as $deposito)
                                                <option value="{{ $deposito->id }}">{{ $deposito->deposito }}</option>
                                            @endforeach
                                        </select>
                                        @error('id_deposito_origen_tf') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Depósito Destino *</label>
                                        <select
                                            wire:model.live="id_deposito_destino_tf"
                                            class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-200"
                                            @if($tieneMultiplesCorralones ? !$id_corralon_destino : !$id_deposito_origen_tf) disabled @endif
                                        >
                                            <option value="">Seleccione depósito destino</option>
                                            @foreach($depositosDestinoTf as $deposito)
                                                <option value="{{ $deposito->id }}">
                                                    {{ $deposito->deposito }}{{ !$tieneMultiplesCorralones ? ' — ' . $deposito->corralon->descripcion : '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('id_deposito_destino_tf') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                        ...
                                        @elseif(!$tieneMultiplesCorralones && !$id_deposito_origen_tf)
                                            <p class="text-xs text-gray-500 mt-1">Primero seleccione un depósito de origen</p>
                                        @endif
                                    </div>
                                </div>

                                @if($id_deposito_origen_tf)

                                    <!-- Búsqueda de maquinarias -->
                                    <div class="relative">
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Agregar Maquinarias *</label>
                                        <div class="relative">
                                            <input
                                                type="text"
                                                wire:model.live="search_maquinaria_transferencia"
                                                wire:focus="mostrarListaTransferencia"
                                                placeholder="Buscar maquinaria por nombre o categoría..."
                                        ...
                                        @if($mostrar_lista_transferencia && $maquinarias_disponibles_transferencia->count() > 0)
                                            <div class="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto">
                                                <ul class="py-2">
                                                    @foreach($maquinarias_disponibles_transferencia as $maq)
                                                        @php $yaAgregada = collect($maquinarias_transferencia)->contains('id', $maq->id); @endphp
                                                        <li>
                                                            <button
                                                                type="button"
                                                                wire:click="agregarMaquinariaTransferencia({{ $maq->id }})"
                                                                class="w-full text-left px-4 py-3 hover:bg-purple-50 transition-colors duration-150 border-b border-gray-100 last:border-b-0 {{ $yaAgregada ? 'opacity-50 cursor-not-allowed' : '' }}"
                                                                @if($yaAgregada) disabled @endif
                                                            >
                                                                <div class="flex justify-between items-start">
                                                                    <div class="flex-1">
                                                                        <div class="text-sm font-medium text-gray-900 flex items-center gap-2">
                                                                            {{ $maq->maquinaria }}
                                                                            @if($yaAgregada)
                                                                                <span class="text-xs text-purple-600">(agregada)</span>
                                                                            @endif
                                                                        </div>
                                                                        <div class="text-xs text-gray-500 mt-1">
                                                                            <span class="px-2 py-0.5 bg-blue-100 text-blue-700 rounded">{{ $maq->categoriaMaquinaria->nombre }}</span>
                                                                        </div>
                                                                    </div>
                                                                    <div class="text-right ml-3">
                                                                        <div class="text-sm font-semibold text-green-600">{{ $maq->cantidad_disponible }}</div>
                                        ...
                                        @if($mostrar_lista_transferencia && $search_maquinaria_transferencia && $maquinarias_disponibles_transferencia->count() == 0)
                                            <div class="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg">
                                                <div class="px-4 py-8 text-center text-gray-400">
                                                    <svg class="w-12 h-12 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                    </svg>
                                                    <p class="text-sm">No se encontraron maquinarias con unidades disponibles</p>
                                                </div>
                                            </div>
                                        @endif

                                        @error('maquinarias_transferencia') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                    </div>

                                    <!-- Lista de maquinarias seleccionadas -->
                                    @if(count($maquinarias_transferencia) > 0)
                                        <div class="border border-gray-200 rounded-xl p-4">
                                            <h4 class="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                                                <svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                                                </svg>
                                                Maquinarias a transferir ({{ count($maquinarias_transferencia) }})
                                            </h4>
                                            <div class="space-y-3">
                                                @foreach($maquinarias_transferencia as $index => $item)
                                                    <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg @error("maquinarias_transferencia.{$index}.cantidad") bg-red-50 border border-red-200 @enderror">
                                                        <div class="flex-1">
                                                            <div class="text-sm font-medium text-gray-900">{{ $item['nombre'] }}</div>
                                                            <div class="text-xs text-gray-500 mt-0.5">
                                                                {{ $item['categoria'] }} • Disponible:
                                                                <span class="font-semibold text-green-600">{{ $item['cantidad_disponible'] }} unidades</span>
                                                            </div>
                                                        </div>
                                                        <div class="w-36">
                                                            <input
                                                                ...
                                                                wire:model.live="maquinarias_transferencia.{{ $index }}.cantidad"
                                                                placeholder="Cantidad"
                                                                class="w-full px-3 py-2 text-sm rounded-lg transition-all duration-200
                                                                    @error("maquinarias_transferencia.{$index}.cantidad")
                                                        ...
                                                        <button
                                                            type="button"
                                                            wire:click="removerMaquinariaTransferencia({{ $index }})"
                                    ...
                                    @else
                                        <div class="text-center py-8 text-gray-400 border border-dashed border-gray-300 rounded-xl">
                                            ...
                                            <p class="text-sm font-medium">No hay maquinarias seleccionadas</p>
                                            <p class="text-xs mt-1">Busque y agregue maquinarias para transferir</p>
                                        </div>
                                    @endif
                                ...
                                @else
                                    <div class="text-center py-12 text-gray-400">
                                        ...
                                        <p class="text-xs mt-1">Para comenzar a agregar maquinarias</p>
                                    </div>
                                @endif
