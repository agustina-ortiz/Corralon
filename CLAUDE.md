...
- **Mensajes de error dentro del modal**: el modal muestra un bloque rojo (debajo del header, en ambos pasos) que renderiza `session('error')` **y** todos los errores de validación (`$errors->all()`). Es necesario porque el bloque de error del tope de la página queda oculto tras el backdrop del modal. Los `addError` de cada línea (`multi_lineas.*.tipo` / `multi_lineas.*.cantidad`) incluyen el nombre del insumo para identificar la línea con problema.
...
- **Filtros globales de la solapa:** corralón, depósito (dependiente del corralón), fecha desde, fecha hasta, **categoría de insumo**, **categoría de maquinaria** y **tipo de movimiento** (todos `wire:model.live`). El helper `depositosConstraint($modulo)` combina permisos del usuario con los filtros corralón/depósito; los widgets `usa_fecha` (movimientos, eventos) respetan el rango.
- **Filtros por categoría / tipo de movimiento:** `insumosQuery()` y `maquinariasQuery()` son las queries base (permisos + corralón/depósito + categoría) que usan todos los widgets de insumos/maquinarias. El filtro de categoría de insumo también acota los widgets de movimientos (vía los insumos incluidos en `movimientos()`), y `filtro_tipo_movimiento` filtra por `movimiento_insumos.id_tipo_movimiento` (select con los tipos `I` e `IM`). `limpiarFiltros()` los resetea y el badge de filtros activos los cuenta.
